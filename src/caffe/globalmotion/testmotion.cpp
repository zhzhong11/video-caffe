...
#include "caffe/globalmotion/testmotion.hpp"
...
motionClassifier::motionClassifier(const string& model_file,
  ...
  CHECK_EQ(input_sixPara->channels(),2);
  CHECK_EQ(input_sixPara->length(),temporary_length_-1);
...
int motionArgmax(const std::vector<float>& v) {
...
int motionClassifier::Classify(std::vector<cv::Mat>& video, int N) {
...
void motionClassifier::SetMean(const string& mean_file, const string& mean_value) {
...
std::vector<float> motionClassifier::Predict(std::vector<cv::Mat>& video) {
  ...
  input_sixPara->Reshape(1, 2,temporary_length_-1,
                       input_geometry_.height, input_geometry_.width);
...
//cal motion time.
  ...
  for(int i=0;i<input_sixPara->length();++i){
	cv::Mat motion;
	extractOpticalFlow(video[i],video[i+1],motion);
	WrapInputLayer(motion,i);
 ...
 * layer. */
void motionClassifier::WrapInputLayer(int j,std::vector<cv::Mat>* input_channels) {
  Blob<float>* input_layer = net_->input_blobs()[0];
...
void motionClassifier::WrapInputLayer(cv::Mat &motion,int j) {
  ...
  for(int h=0;h<height;++h){
	const float* motionData=motion.ptr<float>(h);
	for(int w=0;w<width;++w){
		input_data[h*width+w]=static_cast<float>(motionData[2*w]);
		input_data[h*width+w+width*height*lenght]=static_cast<float>(motionData[2*w+1]);
	}
  }
}

void motionClassifier::Preprocess(const cv::Mat& img,
                            std::vector<cv::Mat>* input_channels) {
