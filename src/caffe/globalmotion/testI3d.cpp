...
#include "caffe/globalmotion/testI3d.hpp"
...
I3dClassifier::I3dClassifier(const string& model_file,
  ...
  CHECK_EQ(net_->num_inputs(), 1) << "Network should have exactly two input.";
...
int ArgmaxN(const std::vector<float>& v) {
...
int I3dClassifier::Classify(const std::vector<cv::Mat>& video, int N) {
...
void I3dClassifier::SetMean(const string& mean_file, const string& mean_value) {
...
std::vector<float> I3dClassifier::Predict(const std::vector<cv::Mat>& video) {
 ...
 * layer. */
void I3dClassifier::WrapInputLayer(int j,std::vector<cv::Mat>* input_channels) {
  Blob<float>* input_layer = net_->input_blobs()[0];
...
void I3dClassifier::Preprocess(const cv::Mat& img,
                            std::vector<cv::Mat>* input_channels) {
