...
#include "caffe/layers/video_lstm_data_layer.hpp"
...
VideoLstmDataLayer<Dtype>::~VideoLstmDataLayer() {
    this->StopInternalThread();
}

template <typename Dtype>
VideoLstmDataLayer<Dtype>::VideoLstmDataLayer(const LayerParameter& param)
    : BaseDataLayer<Dtype>(param),prefetch_(param.data_param().prefetch()),
      prefetch_free_(), prefetch_full_(), prefetch_current_(){
    for(int i=0;i<prefetch_.size();++i){
        prefetch_[i].reset(new LstmBatch<Dtype>());
        prefetch_free_.push(prefetch_[i].get());
    }
}
template <typename Dtype>
void VideoLstmDataLayer<Dtype>::LayerSetUp(
  ...
  for(int i=0;i<prefetch_.size();++i){
      ...
      prefetch_[i]->lstm_flag_.mutable_cpu_data();
  }
#ifndef CPU_ONLY
  if (Caffe::mode() == Caffe::GPU) {
    for (int i = 0; i < prefetch_.size(); ++i) {     
      ...
      prefetch_[i]->lstm_flag_.mutable_gpu_data();
    }
  }
#endif 
...
template <typename Dtype>
void VideoLstmDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>&
    ...
    for (int i = 0; i < prefetch_.size(); ++i) {
        ...
        prefetch_[i]->lstm_flag_.Reshape(label_shape);
    }
}
template <typename Dtype>
void VideoLstmDataLayer<Dtype>::ShuffleVideos() {
...
template <typename Dtype>
void VideoLstmDataLayer<Dtype>::load_batch(LstmBatch<Dtype>* batch){
  ...
  Dtype* prefetch_lstm_flag = batch->lstm_flag_.mutable_cpu_data();
  // datum scales
...
template <typename Dtype>
void VideoLstmDataLayer<Dtype>::InternalThreadEntry(){
#ifndef CPU_ONLY
  cudaStream_t stream;
  if (Caffe::mode() == Caffe::GPU) {
  ...
  try {
    while (!must_stop()) {
      LstmBatch<Dtype>* batch = prefetch_free_.pop();
      load_batch(batch);
#ifndef CPU_ONLY
      if (Caffe::mode() == Caffe::GPU) {
        ...
        batch->lstm_flag_.data().get()->async_gpu_push(stream);
...
template <typename Dtype>
void VideoLstmDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  top[2]->ReshapeLike(prefetch_current_->lstm_flag_);
  top[2]->set_cpu_data(prefetch_current_->lstm_flag_.mutable_cpu_data());
}

INSTANTIATE_CLASS(VideoLstmDataLayer);
REGISTER_LAYER_CLASS(VideoLstmData);
}// namespace caffe
#endif //USE_OPENCV
